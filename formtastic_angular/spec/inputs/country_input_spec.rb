...
describe 'country input' do
  ...
  describe ":priority_countries option" do
    ...
    it "should be passed down to the country_select helper when provided" do
      ...
      semantic_form_for(@new_post) do |builder|
        builder.stub!(:country_select).and_return(Formtastic::Util.html_safe("<select><option>...</option></select>"))
        builder.should_receive(:country_select).with(:country, priority_countries, {}, {:id => "post_country", :required => false, :autofocus => false, :"data-ng-model"=>"post.country"}).and_return(Formtastic::Util.html_safe("<select><option>...</option></select>"))
  ...
  describe "when namespace is provided" do
    ...
    before do
      ...
      concat(semantic_form_for(@new_post, :namespace => 'context2') do |builder|
        builder.stub!(:country_select).and_return(Formtastic::Util.html_safe("<select><option>...</option></select>"))
        builder.should_receive(:country_select).with(:country, [], {}, {:id => "context2_post_country", :required => false, :autofocus => false, :"data-ng-model"=>"post.country"}).and_return(Formtastic::Util.html_safe("<select><option>...</option></select>"))
