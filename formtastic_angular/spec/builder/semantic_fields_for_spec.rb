...
describe 'Formtastic::FormBuilder#fields_for' do
  ...
  context 'within a form_for block' do
    ...
    it 'should correctly model the input field to a nested object' do
      concat(angular_form_for('user', url: '/hello') do |builder|
        concat(builder.semantic_fields_for('address') do |builder|
          concat(builder.input(:title))
        end)
      end)

      output_buffer.should have_tag("form li input[data-ng-model='user.address.title']")
    end
  end
