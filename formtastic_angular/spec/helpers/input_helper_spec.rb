...
describe 'Formtastic::FormBuilder#input' do
  ...
  context "when index is provided" do
    it 'should correctly model the input field to an array' do
      concat(angular_form_for('user', url: '/hello') do |builder|
        concat(builder.semantic_fields_for('addresses', index: 1) do |builder|
          concat(builder.input(:title))
        end)
      end)

      output_buffer.should have_tag("form li input[data-ng-model='user.addresses[1].title']")
    end
  end
